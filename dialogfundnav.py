...
import dialogover
###########################################################################
## Class MyDialogfundnav
###########################################################################

class MyDialogfundnav ( wx.Dialog ):
	
	def __init__( self, parent ):
		wx.Dialog.__init__ ( self, parent, id = wx.ID_ANY, title = u"基金历史净值", pos = wx.DefaultPosition, size = wx.Size( 281,183 ), style = wx.DEFAULT_DIALOG_STYLE )
		...
		self.m_staticText45 = wx.StaticText( self.m_panel6, wx.ID_ANY, u"当天发布前一天数据", wx.DefaultPosition, wx.DefaultSize, 0 )
		...
		# Connect Events
		self.m_button13.Bind( wx.EVT_BUTTON, self.Onfundnavpreday )
		self.m_textCtrl74.Bind( wx.EVT_TEXT_ENTER, self.Inputdate )
		self.m_button18.Bind( wx.EVT_BUTTON, self.Onfundnavoneday )
	...
	def Onfundnavpreday( self, event ):
	...
	def Onfundnavoneday( self, event ):
		inputdate=self.m_textCtrl74.GetValue()
		fund.fundnav(inputdate)
		#print "over"
		app = wx.App()
		frame = dialogover.MyDialogover(parent=None)
		frame.Show()
...
if __name__ == '__main__':
	#app = wx.PySimpleApp()
	app = wx.App()
	frame = MyDialogfundnav(parent=None)
	frame.Show()
	app.MainLoop()	
